...
class ContentsRepository extends LocalStorageRepository {
  ...
  void updateFavoriteContent(List favoriteContentList) async {
    await this
        .storeValue(MY_FAVOIRTE_STORE_KEY, jsonEncode(favoriteContentList));
  }

  addMyFavoriteContent(Map content) async {
  ...
  isMyFavoritecontents(String cid) async {
    bool isMyFavoriteContets = false;
    ...
    if (json == null || !(json is List)) {
      ...
      for (dynamic data in json) {
        if (data["cid"] == cid) {
          isMyFavoriteContets = true;
          break;
        }
      }
    }
    return isMyFavoriteContets;
  }
}
